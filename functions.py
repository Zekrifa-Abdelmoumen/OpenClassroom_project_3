...
def display_factorial_planes_3D(X_projected, n_comp, pca, axis_ranks, labels=None, alpha=1, illustrative_var=None):
    for d1,d2,d3 in axis_ranks:
        if d3 < n_comp:
            ...
            plt.xlabel('F{} ({}%)'.format(d1+1, round(100*pca.explained_variance_ratio_[d1],1)))
            plt.ylabel('F{} ({}%)'.format(d2+1, round(100*pca.explained_variance_ratio_[d2],1)))
            #plt.zlabel('F{} ({}%)'.format(d3+1, round(100*pca.explained_variance_ratio_[d3],1)))

            plt.title("Projection des individus (sur F{} et F{} et F{})".format(d1+1, d2+1, d3+1))
            plt.show(block=False)
